...
MODSIDENTIFIER=$(xpath -q -e "/mods:mods/mods:identifier[@type='local']/text()" "$D/$MODSXML/$B-MODS.xml"); if [ ! $MODSIDENTIFIER == $B ]; then echo "Fejl i MODS-fil - identifier er forkert. Forventet $B, var $MODSIDENTIFIER"; fi
...
FILENAMESUFFIX=$(echo $B|rev|cut -d- -f1|rev|cut -d. -f1|sed -e 's/^0*//'); TIFFSUFFIX=$(echo $MODSTIFFFIL|cut -d_ -f2|cut -d. -f1|sed -e 's/^0*//'); if [ "$FILENAMESUFFIX" != "$TIFFSUFFIX" ]; then echo "Fejl i MODS-fil - MODS-fil slutter med $FILENAMESUFFIX, men refereret TIFF-fil slutter med $TIFFSUFFIX."; fi
